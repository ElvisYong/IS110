...
def multiply(polynomials):
    ...
    while len(polynomials) > 1:
        ...
        polynomials.append(multiply_polynomials(p1, p2))
    
    return [0] if [0] in polynomials else polynomials[0]


def multiply_polynomials(p1, p2):
    longer_polynomial = list(p1 if len(p1) > len(p2) else p2)
    shorter_polynomial = list(p2 if len(p2) < len(p1) else p1)
    if p1 == p2:
        longer_polynomial = list(p2)
        shorter_polynomial = list(p1)

    max_power = len(longer_polynomial) + (len(shorter_polynomial) )

    res = [0] * (max_power -1)
    for i in range(len(shorter_polynomial)):
        for j in range(len(longer_polynomial)):
            res[i + j] += shorter_polynomial[i] * longer_polynomial[j]
